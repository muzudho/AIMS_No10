function t3_OnStart ()
	...
	-- 1.まず画像を読み込む
	G.background   = loadGraphic("gfx/Bg_t3.png");
	...
	--ページ数
		str = currentPage( 1, VIEW_LAST, viewCursorRow*VIEW_COLUMNS+viewCursorColumn ) .."／".. totalPage( VIEW_LAST, #data)
...
function t3_OnStep ()
    ...
    if( 1 == getJoyPressCount( BUTTON_TRIG3 ) ) then
	    --３ボタン（前ページ・端止まり）
    	--境界内チェック
    	local dataCursor = viewHeadPage*VIEW_ROWS*VIEW_COLUMNS -VIEW_ROWS*VIEW_COLUMNS + viewCursorRow*VIEW_COLUMNS+viewCursorColumn
    ...
    if( flg_Head==1 )then
		...
		columnNumber = viewHeadPage*VIEW_ROWS*VIEW_COLUMNS + 1
		if( 1 <= columnNumber and columnNumber <= #data)then
			A.msg1   = createTextActor( F.font1, data[columnNumber],  8.5*32,  3*32, 11 );
		end

		if(isAlive(A.msg2))then
			vanish(A.msg2);
		end
		columnNumber = viewHeadPage*VIEW_ROWS*VIEW_COLUMNS + 2
		if( 1 <= columnNumber and columnNumber <= #data)then
			A.msg2   = createTextActor( F.font1, data[columnNumber],  9.5*32,  3*32, 11 );
		end
		
		if(isAlive(A.msg3))then
			vanish(A.msg3);
		end
		columnNumber = viewHeadPage*VIEW_ROWS*VIEW_COLUMNS + 3
		if( 1 <= columnNumber and columnNumber <= #data)then
			A.msg3   = createTextActor( F.font1, data[columnNumber], 10.5*32,  3*32, 11 );
		end
		
		if(isAlive(A.msg4))then
			vanish(A.msg4);
		end
		columnNumber = viewHeadPage*VIEW_ROWS*VIEW_COLUMNS + 4
		if( 1 <= columnNumber and columnNumber <= #data)then
			A.msg4   = createTextActor( F.font1, data[columnNumber],  8.5*32,  5*32, 11 );
		end
		
		if(isAlive(A.msg5))then
			vanish(A.msg5);
		end
		columnNumber = viewHeadPage*VIEW_ROWS*VIEW_COLUMNS + 5
		if( 1 <= columnNumber and columnNumber <= #data)then
			A.msg5   = createTextActor( F.font1, data[columnNumber],  9.5*32,  5*32, 11 );
		end
		
		if(isAlive(A.msg6))then
			vanish(A.msg6);
		end
		columnNumber = viewHeadPage*VIEW_ROWS*VIEW_COLUMNS + 6
		if( 1 <= columnNumber and columnNumber <= #data)then
			A.msg6   = createTextActor( F.font1, data[columnNumber], 10.5*32,  5*32, 11 );
		end
		
		if(isAlive(A.msg7))then
			vanish(A.msg7);
		end
		columnNumber = viewHeadPage*VIEW_ROWS*VIEW_COLUMNS + 7
		if( 1 <= columnNumber and columnNumber <= #data)then
			A.msg7   = createTextActor( F.font1, data[columnNumber],  8.5*32,  7*32, 11 );
		end
		
		if(isAlive(A.msg8))then
			vanish(A.msg8);
		end
		columnNumber = viewHeadPage*VIEW_ROWS*VIEW_COLUMNS + 8
		if( 1 <= columnNumber and columnNumber <= #data)then
			A.msg8   = createTextActor( F.font1, data[columnNumber],  9.5*32,  7*32, 11 );
		end
		
		if(isAlive(A.msg9))then
			vanish(A.msg9);
		end
		columnNumber = viewHeadPage*VIEW_ROWS*VIEW_COLUMNS + 9
		if( 1 <= columnNumber and columnNumber <= #data)then
			A.msg9   = createTextActor( F.font1, data[columnNumber], 10.5*32,  7*32, 11 );
		end
